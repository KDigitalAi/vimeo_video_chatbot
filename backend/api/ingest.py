...
from backend.modules.vector_store import create_and_upload_to_supabase, check_duplicate_video, delete_video_embeddings
...
from backend.core.validation import VideoIngestRequest, VideoIngestResponse, ErrorResponse
...
async def ingest_video(
    ...
    try:
        ...
        # Step 1: Check for duplicates (unless force re-ingestion)
        if not request.force_transcription:
        ...
        try:
            ...
            video_title = meta.get("name") or meta.get("title") or f"video_{video_id}"
        ...
        # Step 3: Try to get captions from Vimeo
        segments = get_transcript_segments_from_vimeo(video_id)
        if segments:
            transcription_method = "captions"

        # Step 4: Fallback to Whisper if no captions or forced
        ...
        # Step 6: Delete existing embeddings if force re-ingestion
        ...
        # Step 8: Create chunks with custom parameters if provided
        try:
        ...
        # Step 9: Upload embeddings to Supabase with retry logic
        try:
            vs = create_and_upload_to_supabase(chunks)
